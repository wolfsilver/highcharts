...
// encapsulated variables
    ...
    Color.prototype = {
        ...
        get: function (format) {
            ...
            // it's NaN if gradient colors on a column chart
                ...
                if (format === 'hex' && rgba[3] === 1) {
                    ret = '#' + rgba[0].toString(16) + rgba[1].toString(16) + rgba[2].toString(16);
                }else if (format === 'rgb' || (!format && rgba[3] === 1)) {
        ...
        */
        ...
        shadow: function (shadowOptions, group, cutOff) {
            ...
            if (shadowOptions) {
                ...
                if (shadowOptions.insetShadow) {
                    this.renderer.insetShadow();
                    ...
                    attr(shadow, {
                        ...
                        'fill': shadowOptions.color || 'black',
                        'fill-opacity': 0.5,
    ...
    SVGRenderer.prototype = {
        ...
        */
        ...
        insetShadow: function (){
            if (!!document.querySelector('#inset-shadow')) {
                return;
            }
            var filter = this.createElement('filter').attr({
                    id: 'inset-shadow'
                }).add(this.defs),
                feComponentTransfer = this.createElement('feComponentTransfer').attr({
                    'in': "SourceGraphic",
                    result: "inset-selection"
                }).add(filter),
                feFuncA1 = this.createElement('feFuncA').attr({
                    type: "discrete",
                    tableValues: "0 1 1 1 1 1"
                }).add(feComponentTransfer),
                feComponentTransfer2 = this.createElement('feComponentTransfer').attr({
                    "in": "SourceGraphic",
                    result: "original-no-fill"
                }).add(filter),
                feFuncA2 = this.createElement('feFuncA').attr({
                    type: "discrete",
                    tableValues: "0 0 1"
                }).add(feComponentTransfer2),
                feColorMatrix = this.createElement('feColorMatrix').attr({
                    type: "matrix",
                    "in": "original-no-fill",
            ...
            for (var i = 0; i < 8; i++) {
                ...
                this.createElement('feGaussianBlur').attr({
                    "in": "new-source-alpha",
                    result: "blur" + i,
            ...
            var feMerge = this.createElement('feMerge').attr({
                    result: "blur"
                }).add(filter);
            for (var i = 0; i < 8; i++) {
                this.createElement('feMergeNode').attr({
                    "in": "blur" + i,
                    mode: "normal"
                }).add(feMerge);
            }

            this.createElement('feComposite').attr({
                    operator: "in",
                    "in": "inset-selection",
                    in2: "blur",
                    result: "inset-blur"
    ...
    */
        ...
        translate: function (positions) {
            ...
            for (i = 0; i < len; i++) {
                ...
                point.slicedTranslation = {
                    ...
                    translateX: (mathCos(angle) * slicedOffset),
                    translateY: (mathSin(angle) * slicedOffset)
                };
                ...
                if (point.y !== null) {
                    ...
                    if (graphic) {
                        ...
                        point.graphic = graphic = renderer[point.shapeType](shapeArgs)
                            ...
                            .add(series.group)
                            .shadow(extend(shadow, point.shadow), shadowGroup);
                    }
                }
            });
    ...
    extend(Point.prototype, {
        ...
        setState: function (state, move) {
            ...
            // brighten color
            if (state == 'hover' && point.borderColor) {
                pointAttr.stroke = Highcharts.Color(point.borderColor).brighten(0.2).get('hex');
            }
